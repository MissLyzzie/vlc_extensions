...
function descriptor()
    return {
        title = "Move to folder extension linux",
        version = "0.1",
        author = "lyz",
        shortdesc = "➡📂 Move to folder linux",
...
function activate()
    ...
    line = line + 1
    r7_folder_path_input = d:add_text_input(paths[line], 2, line+1)
    r7_move_button = d:add_button("➡ Move to folder", r7_move_to_folder, 1, line+1)
...
function r7_move_to_folder()
    move_to_folder(r7_folder_path_input)
end
...
function move_to_folder(folder_path_input)
    ...
    if folder_path:len() > 0 then
        ...
        while folder_path:sub(1, 3) == "../" do
            folder_parent = file_folder:sub(0, -2)
            file_folder = folder_parent:match("(.*/)")
            folder_path = folder_path:sub(4)
        end
     
        vlc.msg.info("File folder : " .. file_folder)
        vlc.msg.info("File name : " .. file_name)
        ...
        if (retval == nil) then
            ...
            error_label:set_text("✔ Sucess: " .. file_name:sub(1,20))
        end
        vlc.playlist.play()
    end
end
...
function save_paths()
    ...
    if file_write then
        ...
        file_write:write(r7_folder_path_input:get_text().."\n")
        file_write:close()
    end
end
